...
public class GameSetupUIState extends UI {
	...
	public void initialize() {
		...
		player1 = UI.createJPanelWithBackground(Main.createImageIcon("oceanbg.png", 300, 300).getImage());
	...
	public SquareLabel[] searchForHighlightableLabel(SquareLabel startingLabel) {
		...
		/*
		 * int failedAttempt = 0; if(shipPlacingDirection.equals("down")) { //If
		 * the ship placing direction is down if((y + 1 <= 7) &&
		 * !boardLabel[y+1][x].getSquare().isOccupied()) { //If the next down
		 * square exists and is not occupied by a ship highlightable[1] =
		 * boardLabel[y+1][x]; //Else trying to add the above next square } else
		 * if((y - ++failedAttempt >= 0) &&
		 * !boardLabel[y-failedAttempt][x].getSquare().isOccupied()){ //If the
		 * next above square exists and is not occupied by a ship
		 * highlightable[1] = boardLabel[y-failedAttempt][x]; } else return
		 * null; //Else return null if((y + 2 <= 7) &&
		 * !boardLabel[y+2][x].getSquare().isOccupied()) { //If the next down
		 * square exists and is not occupied by a ship highlightable[2] =
		 * boardLabel[y+2][x]; //Else trying to add the above next square } else
		 * if((y - ++failedAttempt >= 0) &&
		 * !boardLabel[y-failedAttempt][x].getSquare().isOccupied() &&
		 * (failedAttempt == 0)){ //If the next above square exists and is not
		 * occupied by a ship (and must be consecutive) highlightable[2] =
		 * boardLabel[y-failedAttempt][x]; } else return null; //Else return
		 * null if((y + 3 <= 7) && !boardLabel[y+3][x].getSquare().isOccupied())
		 * { //If the next down square exists and is not occupied by a ship
		 * highlightable[3] = boardLabel[y+3][x]; //Else trying to add the above
		 * next square } else if((y - ++failedAttempt >= 0) &&
		 * !boardLabel[y-failedAttempt][x].getSquare().isOccupied() &&
		 * (failedAttempt == 0)){ //If the next above square exists and is not
		 * occupied by a ship (and must be consecutive(failed = 0))
		 * highlightable[3] = boardLabel[y-failedAttempt][x]; } else return
		 * null; //Else return null
		 * 
		 * } else { //If the ship placing direction is right if((x + 1 <= 7) &&
		 * !boardLabel[y][x+1].getSquare().isOccupied()) { //If the next down
		 * square exists and is not occupied by a ship highlightable[1] =
		 * boardLabel[y][x+1]; //Else trying to add the above next square } else
		 * if((x - ++failedAttempt >= 0) &&
		 * !boardLabel[y][x-failedAttempt].getSquare().isOccupied()){ //If the
		 * next above square exists and is not occupied by a ship
		 * highlightable[1] = boardLabel[y][x-failedAttempt]; } else return
		 * null; //Else return null if((x + 2 <= 7) &&
		 * !boardLabel[y][x+2].getSquare().isOccupied()) { //If the next down
		 * square exists and is not occupied by a ship highlightable[2] =
		 * boardLabel[y][x+2]; //Else trying to add the above next square } else
		 * if((x - ++failedAttempt >= 0) &&
		 * !boardLabel[y][x-failedAttempt].getSquare().isOccupied()){ //If the
		 * next above square exists and is not occupied by a ship
		 * highlightable[2] = boardLabel[y][x-failedAttempt]; } else return
		 * null; //Else return null if((x + 3 <= 7) &&
		 * !boardLabel[y][x+3].getSquare().isOccupied()) { //If the next down
		 * square exists and is not occupied by a ship highlightable[3] =
		 * boardLabel[y][x+3]; //Else trying to add the above next square } else
		 * if((x - ++failedAttempt >= 0) &&
		 * !boardLabel[y][x-failedAttempt].getSquare().isOccupied()){ //If the
		 * next above square exists and is not occupied by a ship
		 * highlightable[3] = boardLabel[y][x-failedAttempt]; } else return
		 * null; //Else return null }
		 */
	}
